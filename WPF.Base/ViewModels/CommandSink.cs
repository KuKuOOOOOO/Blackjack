...
namespace WPF.ViewModels
{
  ...
  public class CommandSink : ICommandSink
  {
    ...
    public void RegisterCommand(ICommand command, Predicate<object> canExecute, Action<object> execute)
    {
      ...
      this._commandToCallbacksMap[command] = new CommandCallbacks(canExecute, execute);
    }
    ...
    public void UnregisterCommand(ICommand command)
    {
      ...
      if (this._commandToCallbacksMap.ContainsKey(command)) this._commandToCallbacksMap.Remove(command);
    }
    ...
    public virtual bool CanExecuteCommand(ICommand command, object parameter, out bool handled)
    {
      ...
      if (this._commandToCallbacksMap.ContainsKey(command))
      {
        handled = true;
        return this._commandToCallbacksMap[command].CanExecute(parameter);
      }
      else
      {
    ...
    public virtual void ExecuteCommand(ICommand command, object parameter, out bool handled)
    {
      ...
      {
        handled = true;
        this._commandToCallbacksMap[command].Execute(parameter);
      }
      else
      {
        handled = false;
